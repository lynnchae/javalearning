...
+ 垃圾回收器
  ...
  + **老年代收集器**：
    ...
    + CMS（ConcurrentMarkSweep）：只有ParNew可以搭配使用，标记-清除算法，并发低停顿，注重用户响应
      ...
      + 初始标记：标记GC Roots关联的对象（STW）
      + 并发标记：标记GC Roots关联对象的引用对象
      + 重新标记：在并发标记阶段，应用的线程可能产生新的垃圾，所以需要重新标记（STW ）
...
问题1 ： 定时同步订单状态给到商户，总共三次，时间间隔为1分钟、30分钟、60分钟

1.1 方案1
    
    定时任务扫描数据库
    如果数据插入频率较高，轮询的频率如何设定
    太高了则影响数据入库，太低了，订单状态同步的时效性无法保证
    而且当订单数据量很大时，扫描数据库加载到内存太占资源了

1.2 方案2

    使用ScheduledExecutorService，此方法简单，但如果系统重启恢复后，需要记录之前任务的执行状态，失败已经未执行的任务都需要
    重新加入到线程池中，同样会存在内存消耗较大的问题

1.3 方案3
    
    使用Redis ZSET有序集合时间，集合中存放订单号对象，分值对应为未来要执行的时间戳。
    zrangeWithScores获取到第一个节点，如果节点的分值小于当前时间戳，则执行订单状态的同步，然后从zset中zrem删除。
    zrangeWithScores和zrem需要在一个事务中执行，然而在分布式环境下，可能会从redis中拿到同样的数据，重复执行和删除。
    如果使用redis事务，或者redis分布式锁，又增加了系统复杂度。
   
1.4 最终方案

    基于rabbitmq消息存活时间TTL，以及消息过期后的dead-letter-exchage，实现延时队列。
    当消息过期后路由到新的队列中，分别设置1分钟、30分钟和60分钟这三个死信队列。
    该方案前提是保证的rabbitmq消息的可靠性。
